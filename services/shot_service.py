...
class ShotService:
	...
	def split_scene_into_cinematic_shots(self, scene_id: int, raw_scene_text: str) -> list[ShotModel]:
		...
		for bp in blueprints:
			...
			# Kiểm tra xem cú máy này đã tồn tại trong DB chưa để tránh ghi đè dữ liệu cũ
			existing_shot = self.db.query(ShotModel).filter(ShotModel.id == shot_db_id).first()
			if existing_shot:
				created_shots.append(existing_shot)
				continue

			# Khởi tạo thực thể dữ liệu sạch, mặc định ở trạng thái DRAFT của LTX Workspace
			shot_record = ShotModel(
				...
				status="draft",
				duration=bp["duration"], # Gán chính xác mốc thời gian 5s, 4s, 4s của ảnh mẫu
				seed="23561",
